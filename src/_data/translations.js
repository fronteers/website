module.exports = {
  en: {
    baseUrl: "https://www.fronteers.nl/en",
    siteName: "Fronteers",
    headTitle: "Fronteers — professional association for front-end developers",
    metaTitle: "Fronteers — professional association for front-end developers",
    metaDescription:
      "Fronteers is the non-profit professional association of Dutch-speaking front-end developers.",
    ariaTitleTranslateNav: "Language",
    location: "Location",
    locationUnknown: "to be determined",
    jobreadmore: "Read more",
    blogreadmore: "Read more",
    moreInformation: "More information",
    readmore: "Read more",
    about: "about",
    for: "for",
    goToVimeo: "Go to our Vimeo",
    signUp: "Sign up",
    signUpMail: "Sign up via email",
    signUpMeetup: "View on Meetup.com",
    signUpClosed: "Show details",
    opensubmenu: "Open submenu",
    closesubmenu: "Close submenu",
    skiplink: "Skip to main content",
    editThisPage: "Edit this page on GitHub",
    activitySingular: "Activity",
    activityDate: "Event date",
    subscribeToCalendarDescription: "Want to see all of our events in your own calendar program?",
    subscribeToCalendar: "Subscribe to our calendar",
    noOpenVacancies:
      "<h2>There are no open job offers right now.</h2> Does your company have a position open? <a href='/en/jobs/post-job-opening/'>Post a job opening</a>! Open internships positions is free. And because of the launch of the new website, posting a vacancy for a job or freelance assignment is free until the end of 2024!",
    footerNavigationLabel: "Footer navigation",
    blockTitles: {
      homeHero: "We unite front-end developers",
      upcomingActivities: "Upcoming activities",
      latestBlogs: "From our blog",
      proudMembers: "Get to know our members",
      conferenceBanner: "Get a taste of our conference",
      membersSpecial: "Member's Special",
      joinUsTitle: "Join our community",
      joinUs: "Join us",
      joinUsPath: "/en/join-us/",
      joinUsDescription:
        "Become a member of the Fronteers community and contribute to the professionalization of the front-end profession.",
      activeVacancies: "Our job board",
      viewAllEvents: "View all events",
      viewOurBlogs: "View all our blogposts",
      viewOurMembers: "View all our members",
      viewOurJobs: "View all known vacancies",
      jobsPath: "/en/jobs/",
      ourMembersPath: "/en/members/",
    },
    socialMedia: {
      title: "Social media",
      slackTitle: "Slack",
      slackText: `The Fronteers community is highly active on Slack. <a href="https://join.slack.com/t/fronteersnl/shared_invite/zt-1m0mbjbkh-LyrZgCPr1JzWBeASuTcnog">Sign up to the Fronteers Slack</a> and join the conversation about everything related to front-end development.`,
      linkedinTitle: "LinkedIn",
      linkedinText: `Follow us on <a href="https://www.linkedin.com/company/fronteers">LinkedIn</a> and get updates about what we're up to.`,
      elsewhereTitle: "Elsewhere",
      elsewhereText: `You can follow us through <a href="https://front-end.social/@fronteers">Mastodon</a> or <a href="https://bsky.app/profile/fronteers.bsky.social">Bluesky</a>. We try to share interesting links to keep you up to date. Or check out our profile on <a href="https://www.meetup.com/fronteers-nl/">Meetup</a>.`,
    },
    joinUsSlogan: "Hey front-ender!<br/> Support our community and become a Fronteers member!",
    member_specialty: "Specialty",
    job_category: "Category",
    previous: "Previous",
    next: "Next",
    all: "All",
    specialties: "Specialties",
    specialty: "specialty",
    category: "category",
    filter: "Filter",
    allCategories: "Show all",
    membersURL: "en/members",
    freelancersURL: "en/freelancers",
    mentorsURL: "en/mentors",
    back: "Go back",
    is_a: "is a",
    worktitle: "Employers, pay attention",
    isAvailable: "is available",
    hasSpecialisms: "has the following specialisms:",
    lookingForEmployment: "is looking for a job",
    allMembers: "Back to our members list",
    allFreelancers: "All freelancing Fronteers members",
    allMentors: "All Fronteers members who mentor",
    blogsURL: "en/blog",
    activitiesURL: "en/activities",
    jobsURL: "en/jobs",
    conferenceURL: "en/conference",
    olderArticles: "Older articles",
    newerArticles: "Newer articles",
    nextEvent: "Next event: ",
    lastEvent: "Last event: ",
    latestBlog: "Latest blog ",
    page: "Page",
    pagination: "Pagination",
    emptyStateActivities:
      "We currently have no upcoming events, check back soon for more updates.",
    upcomingActivitiesTitle: "Upcoming activities",
    activitiesTitle: "Activities",
    pastActivitiesTitle: "Past activities",
    pastActivitiesURL: "past",
    viewPastEvents: "View past events",
    scheduled_date: "Publish Date",
    scheduled_title: "Title",
    scheduled_author: "Author",
    scheduled_pagetype: "Type",
    scheduled_something:
      "Posts that are future-dated, and will be shared later.",
    scheduled_nothing: "Nothing to show here!",
    post: "Blog article",
    member: "Members profile",
    activity: "Event announcement",
    job: "Job opening",
    externalEvent: "Please note: This is an event that is not organised by Fronteers.",
    externalEventDiscount: "A discount to this event is available exclusively for Fronteers members. Please contact the board of Fronteers for more information through bestuur@fronteers.nl.",
    of: "of",
    shown: "shown"
  },
  nl: {
    baseUrl: "https://www.fronteers.nl/nl",
    siteName: "Fronteers",
    headTitle: "Fronteers — vakvereniging voor front-end developers",
    metaTitle: "Fronteers — vakvereniging voor front-end developers",
    metaDescription:
      "Fronteers is een vereniging met volledige rechtsbevoegdheid, die in oktober 2007 is opgericht, met als doel de professionalisering van het beroep van web-front-end ontwikkelaar.",
    ariaTitleTranslateNav: "Taal",
    jobreadmore: "Bekijk vacature",
    blogreadmore: "Lees meer",
    moreInformation: "Meer informatie",
    readmore: "Lees meer",
    about: "over",
    for: "voor",
    goToVimeo: "Ga naar ons profiel op Vimeo",
    location: "Locatie",
    locationUnknown: "nog te bepalen",
    signUp: "Meld je aan",
    signUpMail: "Meld je aan via email",
    signUpMeetup: "Meld je aan op Meetup.com",
    signUpClosed: "Bekijk details",
    opensubmenu: "Open submenu",
    closesubmenu: "Sluit submenu",
    skiplink: "Sla het menu over",
    editThisPage: "Pas deze pagina aan via GitHub",
    activitySingular: "Activiteit",
      activityDate: "Datum activiteit",
      subscribeToCalendarDescription: "Alles uit deze agenda in je eigen kalenderprogramma zien?",
    subscribeToCalendar: "Abonneer je op onze agenda",
    noOpenVacancies:
      "<h2>Momenteel zijn er geen openstaande vacatures.</h2> Heeft jouw bedrijf een leuke positie open staan? <a href='/nl/werk-en-freelance/vacature-plaatsen/'>Plaats een vacature</a>! Voor stageplekken is dit gratis. En vanwege de lancering van de nieuwe website is het plaatsen van een vacature voor een baan of freelance opdracht gratis tot het eind van 2024!",
    footerNavigationLabel: "Footer navigatie",
    blockTitles: {
      homeHero: "We unite front-end developers",
      upcomingActivities: "Deze activiteiten staan in onze agenda...",
      latestBlogs: "Uit ons blog",
      proudMembers: "Trotse leden",
      conferenceBanner: "Bekijk video's van ons congres",
      membersSpecial: "Speciaal voor leden",
      joinUsTitle: "Sluit je aan bij onze community",
      joinUs: "Word lid",
      joinUsPath: "/nl/word-lid/",
      joinUsDescription:
        "Word lid van de Fronteers community en draag hiermee bij aan de professionalisering van het front-end vak.",
      activeVacancies: "Vacatures",
      viewAllEvents: "Bekijk alle activiteiten",
      viewOurBlogs: "Bekijk alle blogposts",
      viewOurMembers: "Bekijk alle leden",
      viewOurJobs: "Bekijk alle geplaatste vacatures",
      jobsPath: "/nl/werk-en-freelance/",
      ourMembersPath: "/nl/leden/",
    },
    socialMedia: {
      title: "Sociale media",
      slackTitle: "Slack",
      slackText: `Fronteers heeft een actieve community op Slack. <a href="https://join.slack.com/t/fronteersnl/shared_invite/zt-1m0mbjbkh-LyrZgCPr1JzWBeASuTcnog">Meld je aan voor onze Slack</a> en praat mee over allerlei zaken rond frontend development.`,
      linkedinTitle: "LinkedIn",
      linkedinText: `Volg ons op <a href="https://www.linkedin.com/company/fronteers">Linkedin</a> en krijg updates over waar we ons mee bezig houden.`,
      elsewhereTitle: "Elders",
      elsewhereText: `Je kunt Fronteers volgen via <a href="https://front-end.social/@fronteers">Mastodon</a> of <a href="https://bsky.app/profile/fronteers.bsky.social">Bluesky</a>. Vragen en opmerkingen over wat we doen kun je daar kwijt. Of bekijk ons profiel op <a href="https://www.meetup.com/fronteers-nl/">Meetup</a>.`,
    },
    joinUsSlogan: "Hey front-ender!<br/> Vergroot je netwerk en kom erbij!",
    member_specialty: "Freelancers met specialiteit",
    job_category: "Werk in de categorie",
    previous: "Vorige",
    next: "Volgende",
    all: "Alle",
    specialty: "specialiteit",
    specialties: "Specialiteiten",
    category: "categorie",
    filter: "Filter...",
    allCategories: "Toon alles",
    membersURL: "nl/leden",
    freelancersURL: "nl/freelancers",
    mentorsURL: "nl/mentors",
    back: "Ga terug",
    is_a: "is een",
    worktitle: "Werkgevers opgelet",
    isAvailable: "is beschikbaar",
    hasSpecialisms: "heeft de volgende specialismes:",
    lookingForEmployment: "is op zoek naar een leuke werkgever",
    allMembers: "Terug naar de ledenlijst",
    allFreelancers: "Bekijk alle freelancers",
    allMentors: "Bekijk alle mentoren",
    blogsURL: "nl/blog",
    activitiesURL: "nl/activiteiten",
    jobsURL: "nl/werk-en-freelance",
    conferenceURL: "nl/congres",
    olderArticles: "Ouder",
    newerArticles: "Nieuwer",
    nextEvent: "Up next: ",
    lastEvent: "Laatste activiteit: ",
    latestBlog: "Laatste blog ",
    page: "Pagina",
    pagination: "Paginering",
    emptyStateActivities:
      "Momenteel zijn er geen nieuwe activiteiten gepland, neem binnenkort gerust terug een kijkje.",
      upcomingActivitiesTitle: "Geplande activiteiten",
    activitiesTitle: "Activiteiten",
    pastActivitiesTitle: "Voorbije activiteiten",
    pastActivitiesURL: "voorbije",
    viewPastEvents: "Bekijk voorbije activiteiten",
    scheduled_date: "Publicatiedatum",
    scheduled_title: "Titel",
    scheduled_author: "Auteur",
    scheduled_pagetype: "Soort",
    scheduled_something:
      "Pagina's die binnenkort publiekelijk zichtbaar worden.",
    scheduled_nothing: "Hier is niets te zien!",
    post: "Blog",
    member: "Ledenprofiel",
    activity: "Activiteit",
    job: "Vacature",
    externalEvent: "Let op: Dit betreft een activiteit die niet door Fronteers wordt georganiseerd.",
    externalEventDiscount: "Er is een korting voor dit evenement beschikbaar, exclusief voor Fronteersleden. Neem contact op met het bestuur van Fronteers via bestuur@fronteers.nl.",
    of: "van",
    shown: "getoond"
  },
};
