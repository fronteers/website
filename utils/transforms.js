...
transforms["convert_markdown_headers"] = function (content, outputPath) {
  if (
    !outputPath ||
    outputPath.endsWith("kitchensink-html.html") ||
  ...
  dom.window.document
    .querySelectorAll(".page-content h5")
    .forEach((h5) => swapHeading(h5, "h6"));
  dom.window.document
    .querySelectorAll(".page-content h4")
    .forEach((h4) => swapHeading(h4, "h6"));
  dom.window.document
    .querySelectorAll(".page-content h3")
    .forEach((h3) => swapHeading(h3, "h5"));
  dom.window.document
    .querySelectorAll(
      `[data-layout="blog-single"] .page-content h2,
  ...
  dom.window.document
    .querySelectorAll(".page-content h1")
    .forEach((h1) => swapHeading(h1, "h3"));
...
transforms["add_table_scope"] = function (content, outputPath) {
  ...
  dom.window.document.querySelectorAll("th").forEach((th) => {
